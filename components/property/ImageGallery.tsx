  ...
  return (
    <>
      ...
      >
          ...
          {images.length > 1 && (
            <>
              <Button
                variant="ghost"
                size="icon"
                className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-background/80 backdrop-blur-sm hover:bg-background shadow-warm opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
                  goToPrevious();
                }}
              ...
              <Button
                variant="ghost"
                size="icon"
                className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-background/80 backdrop-blur-sm hover:bg-background shadow-warm opacity-0 group-hover:opacity-100 transition-opacity duration-300 cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
                  goToNext();
                }}
        ...
        {images.length > 1 && (
          <div
            className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide cursor-pointer"
            role="listbox"
          ...
          >
            {images.map((image, index) => (
              <button
                ...
                className={`relative flex-shrink-0 w-28 h-20 rounded-xl overflow-hidden border-2 transition-[border-color,transform,box-shadow] duration-200 cursor-progress ${
                  index === selectedIndex
